...
Dentro da pasta data voce encontra as pastas ``raw'' e ``processed''. Em outputs voce encontra ``figures'', ``temp'' e ``tables''. Em script, voce encontrara nada (por enquanto). O arquivo com o simbolo do R ``projeto\_caranguejos.Rproj'' eh o seu projeto se R. Toda vez que voce for trabalhar nas suas analises desse projeto, voce iniciar o R (ja no projeto) ao abrir esse arquivo (dando clique duplo nele).
